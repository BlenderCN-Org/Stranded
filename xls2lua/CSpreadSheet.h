...
/*
 * for visual studio 2012 <afxdb.h> <odbcinst.h>
 */
#include <odbcinst.h>
#include <afxdb.h>
...
private:
	...
	bool m_bAppend; // Internal flag to denote newly created spreadsheet
		//or previously created spreadsheet
	bool m_bBackup; // Internal flag to denote status of Backup
	bool m_bExcel; // Internal flag to denote whether file is Excel spreadsheet or
...
m_bAppend(false), m_bBackup(Backup), m_bTransaction(false)
{
	...
	int iPos = m_sFile.ReverseFind('\\');
	if (iPos == -1)
		iPos = m_sFile.ReverseFind('/');
	m_extName = m_sFile.Right(m_sFile.GetLength()-1-iPos);
	iPos = m_extName.ReverseFind('.');
	m_extName = m_extName.Right(m_extName.GetLength() - 1 - iPos);
	m_extName.MakeLower();
	if (m_extName == "xls" || m_extName == "xlsx") // File is an Excel spreadsheet
	{
		m_bExcel = true;
	...
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		GetExcelDriver();
		if (m_sExcelDriver.IsEmpty())
		{
			if(m_extName == "xlsx")
				AfxMessageBox("没有安装Excel驱动，请安装office 2007或更高版本!");
			else
				AfxMessageBox("没有安装Excel驱动，请安装office 2003或更高版本!");
			return;
		}
		...
		if (m_extName == "xls")
			...
			m_sDsn.Format("DRIVER={%s};DBQ=%s;FIL=Excel 12.0 XML", m_sExcelDriver, m_sFile);
...
void CSpreadSheet::GetExcelDriver()
{
	...
	do
	{
		...
		{
			// 2003以下版本 Excel 和 2007以上版本 Excel 有区别
			if(strstr(pszBuf, (LPCTSTR)(m_extName)) != 0)
			{
				// Found !
